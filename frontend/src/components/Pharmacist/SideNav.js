  ...
  const sideNavLinksData = [
    {
      ...
      name: browserLinks.Pharmacist.internalPages.Patientlist,
    },
  ];
  ...
  const renderedSideNavLinks = sideNavLinksData?.map((data, index) => {
    return (
      <div
        key={`${index}-${data?.name}`}
        onClick={() => {
          setActive(`${browserLinks?.Pharmacist?.category}/${data?.name}`);
          const link = data?.name?.split(" ").join("");
          navigate(`${browserLinks?.Pharmacist?.category}/${link}`);
        }}
        className={
          active === `${browserLinks?.Pharmacist?.category}/${data?.name}`
